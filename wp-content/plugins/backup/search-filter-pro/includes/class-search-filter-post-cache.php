...
class Search_Filter_Post_Cache
{
    ...
    public function __construct()
    {
        ...
        //add_action('save_post', array($this, 'super_early_save_post'), -10, 3); //priority of 80 so it runs after the regular S&F form save
    ...
    public function cache_progress()
    {
        ...
        if ($this->cache_options['rc_status'] == "") {//then we need to test for a remote connection
    ...
    public function post_meta_updated($meta_id, $object_id, $meta_key, $meta_value)
    {
        ...
        /*echo "<hr /><pre>";
        echo $meta_id.PHP_EOL;
        echo $object_id.PHP_EOL;
        echo $meta_key.PHP_EOL;
        echo $meta_value.PHP_EOL;

        echo "</pre><hr />";*/
        $post = get_post($object_id);
        //$this->write_log("..entrance via  `post_meta_updated` | $meta_key : " . $post->ID);
        $this->post_updated($post->ID, $post, false);
        //$this->write_log("..post_updated_count: " . $this->post_updated_count);
    ...
    public function object_terms_updated($postID, $terms, $tt_ids, $taxonomy)
    {
        ...
        //$this->write_log("..entrance via  `object_terms_updated`: " . $postID);
        //$this->write_log($terms);
        //$this->write_log($tt_ids);
        //$this->write_log($taxonomy);
    ...
    public function updated_post_meta_updated($meta_id, $object_id, $meta_key, $meta_value)
    {
        ...
        $post = get_post($object_id);
        //$this->write_log("..entrance via  `updated_post_meta_updated` | $meta_key : " . $post->ID);
        $this->post_updated($post->ID, $post, false);
        //$this->write_log("..post_updated_count: " . $this->post_updated_count);
    ...
    {
        //$this->write_log("..entrance via  `attachment_updated`: " . $post->ID);
        $this->post_updated($post->ID, $post, false);
        //$this->write_log("..post_updated_count: " . $this->post_updated_count);
    ...
    public function super_early_save_post($postID, $post, $update)
    {
        //$this->write_log("* super_early_save_post*: " . $post->ID);
    ...
    {
        //$this->write_log("..entrance via  `post_updated_action`: " . $post->ID);
        $this->post_updated($postID, $post, $update);
        //$this->write_log("..post_updated_count: " . $this->post_updated_count);
	...
	public function post_updated( $postID, $post, $update )
	{
        ...
        //$this->write_log("STARTING POST UPDATED FOR: ".$postID);
		...
		if( ! ( wp_is_post_revision( $postID ) && wp_is_post_autosave( $postID ) ) )
		{
			...
			{//then do some checks to see if we need to update the cache for this
				...
				{
                    //$this->write_log("**** RUNNING UPDATE PROCESSES: ".$postID);
                    ...
                    //$this->write_log("time to complete `update_post_cache`: ".round($time_to_complete, 4));
        ...
        //$this->write_log("FINISHING POST UPDATED FOR: ".$postID);
	...
	public function update_post_cache($postID, $post = "", $update_term_cache = true)
	{
        ...
        //Search_Filter_Helper::finish_log("insert_blank_option");
        ...
        //Search_Filter_Helper::start_log("combine a bunch of arrays");
		...
		foreach($unique_keys as $unique_key)
		{
			...
			{ //we shoudl really check for differences in values and only update thos
                ...
                //echo "<br />Combined Terms<br />";
                //var_dump($combined_terms);
                //echo "<hr />";
                ...
                //$combined_terms = array_diff($fields_previous[$unique_key], $fields_added[$unique_key] );
                //var_dump($array_diff);
                /*$this->write_log("COMBINED TERMS ($unique_key)");
                $this->write_log(array(

                    'previous_values' => $fields_previous[$unique_key],
                    'added_values'     => $fields_added[$unique_key],
        ...
        //Search_Filter_Helper::finish_log("combine a bunch of arrays");
        ...
        $this->write_log("beforessss | ID: ".$postID);

        if(empty($field_differences))
        {
            //$this->write_log("NOT UPDATING META");$this->write_log("beforessss | ID: ".$postID);
            ...
            //$wpdb->delete( $this->cache_table_name, array( 'post_id' => $postID, 'field_name' => '', 'field_value' => '', 'post_parent_id' => $post->parent_id ) );
        ...
        //$this->write_log("time to complete `post_update_taxonomies`: ".round($time_to_complete, 4));
        ...
        //$this->write_log("time to complete `post_update_post_meta`: ".round($time_to_complete, 4));

        //Search_Filter_Helper::start_log("insert_blank_option");
        ...
        //$this->write_log("aftersss | ID: ".$postID);
		...
		foreach($field_differences as $filter => $terms)
		{
			...
			if($source!="")
			{
				...
				{
                    $cc++;
                    //Search_Filter_Helper::start_log("delete_term_results");
					...
					//$wpdb->delete( $this->term_results_table_name, $delete_args);
                    array_push($all_delete_rows, $delete_args);
                    //Search_Filter_Helper::finish_log("delete_term_results", false, false);
                    //Search_Filter_Helper::add_time_running("delete_term_results_total", "delete_term_results");


                    //Search_Filter_Helper::start_log("get_cache_term_ids");
                    ...
                    //Search_Filter_Helper::finish_log("get_cache_term_ids", false, false);
                    //Search_Filter_Helper::add_time_running("get_cache_term_ids_total", "get_cache_term_ids");


					//$save_term_ids = implode("," , $term_ids);
                    //Search_Filter_Helper::start_log("insert_term_results");
					//$this->insert_term_results($filter, $term_value, $term_ids);
                    //Search_Filter_Helper::finish_log("insert_term_results", false, false);
                    //Search_Filter_Helper::add_time_running("insert_term_results_total", "insert_term_results");
				}
        ...
        //Search_Filter_Helper::get_time_running("delete_term_results_total", true, true);

        //Search_Filter_Helper::get_time_running("get_cache_term_ids_total", true, true);
        //Search_Filter_Helper::get_time_running("insert_term_results_total");

        //Search_Filter_Helper::finish_log("field_differences");
        //var_dump($all_delete_rows);
	...
	private function term_results_delete_rows($delete_rows)
    {
        ...
        if(count($delete_rows > 0)) {
            ...
            foreach ($delete_rows as $del_row) {
                ...
                //array_push($sql_where_parts, "('".$del_row['field_name']."', ".$del_row['field_value'].')');
            ...
            //$sql = 'DELETE FROM '.$this->term_results_table_name.' WHERE (field_name, field_value) IN ('.$sql_where_in.')';
	...
	private function get_all_cache_term_ids($cache_terms)
    {
        ...
        if(count($cache_terms > 0)) {
            ...
            foreach($term_results as $term_result)
            {
                ...
                //foreach($field_terms_results as $field_terms_result)
                //{
                //    $field_term_ids[$term_result->post_id] = 1;
                //}
            }

            /*echo "<pre>";
            var_dump($field_term_ids);
            echo "</pre>";*/
