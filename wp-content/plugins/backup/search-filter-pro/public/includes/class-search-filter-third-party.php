...
class Search_Filter_Third_Party
{
    ...
    public function sf_woocommerce_cache_filter_names($field_names, $sfid)
    {
        ...
        if($this->sf_woocommerce_is_woo_variations_query($sfid))
        {
            ...
            /*echo "\r\n************************\r\n";
            var_dump("found: ");
            var_dump($active_taxonomy_names);
            echo "\r\n************************\r\n";*/
    ...
    public function sf_woocommerce_convert_term_results($filters, $cache_term_results, $sfid)
    {
        ...
        //var_dump($cache_term_results);

        foreach($this->woo_meta_keys_added as $woo_tax_name)
        {
            //echo "TAX NAME".$woo_tax_name."<Br />";
        ...
        //var_dump($filters);
        // we want to combine the results we got from our variations (taxonomy, but stored as meta on the variation itelf), and add the result IDs to the taxonomy equivelant
	...
	public function sf_woocommerce_conv_variable_ids($post_ids, $sfid)
    {
        ...
        if($this->sf_woocommerce_is_woo_variations_query($sfid)){
            ...
            $del_val = 13;
            $del_search = array_search($del_val, $post_ids);
            //echo "del search: ".$del_search."\r\n";
            if($del_search !== false)
            {
                unset($post_ids[$del_search]);
            }*/
            //echo "before\r\n";
            //var_dump($post_ids);
